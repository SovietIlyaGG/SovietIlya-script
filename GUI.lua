...
-- v14 FINAL | All Tabs + Radius + Auto Chop
-- =============================================
...
function GUI:Init(Funcs)
    ...
    CircleBtn.MouseButton1Click:Connect(function()
        if not circleMoved then Main.Visible = not Main.Visible end
    end)
    ...
    createToggle(forestContent, "Show Radius (Red Circle)", 22, function(on) Functions:ShowRadius(on) end)
    createToggle(forestContent, "Auto Chop Trees (All)", 54, function(on) Functions:AutoChopTrees(on) end)
    createToggle(forestContent, "ESP", 86, function(on) Functions:ForestESP(on) end)
    createToggle(forestContent, "Auto-Farm (All-in-One)", 118, function(on) Functions:ForestAutoFarm(on) end)
    createToggle(forestContent, "One Hit Kill", 150, function(on) Functions:ForestOneHit(on) end)
    createToggle(forestContent, "Inf Diamonds", 182, function(on) Functions:ForestDiamonds(on) end)

    local itemList = {"Sword", "Axe", "Pickaxe", "Torch", "Armor", "Shield", "Bow"}
    createButton(forestContent, "Dupe Items (at Campfire)", 214, function()
        createPopupWithAmount("Select Item", itemList, function(item, amount) Functions:DupeItem(item, amount) end)
    end)
    createButton(forestContent, "Dupe Chests (at Campfire)", 248, function()
    ...
    createButton(forestContent, "Inf Foods (at Campfire)", 282, function()
    ...
    createButton(forestContent, "Inf Guns (at Campfire)", 316, function()
        createPopupWithAmount("Select Gun", gunList, function(gun, amount) Functions:DupeGun(gun, amount) end)
    end)
    createToggle(forestContent, "Inf Ammo", 350, function(on) Functions:InfAmmo(on) end)
    createButton(forestContent, "Dupe Wood (at Campfire)", 382, function()
