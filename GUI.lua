...
-- v11 FINAL | All Fixed | Music Player Browse
-- =============================================
...
function GUI:Init(Funcs)
    ...
    SubTitle.Text = "@Zamonolit222 Telegram"
    ...
    createToggle(forestContent, "Auto-Farm (All-in-One)", 54, function(on) Functions:ForestAutoFarm(on) end)
    ...
    createButton(forestContent, "Dupe Items (at Campfire)", 150, function()
        createPopupWithAmount("Select Item", itemList, function(item, amount) Functions:DupeItem(item, amount) end)
    end)
    createButton(forestContent, "Dupe Chests (at Campfire)", 184, function()
    ...
    createButton(forestContent, "Inf Foods (at Campfire)", 218, function()
    ...
    createButton(forestContent, "Inf Guns (at Campfire)", 252, function()
    ...
    createButton(forestContent, "Dupe Wood (at Campfire)", 318, function()
    ...
    musicLabel.Text = "-- MUSIC PLAYER (Local Files) --"
    ...
    musicInput.PlaceholderText = "Audio ID"
    musicInput.Text = ""
    ...
    browseBtn.Parent = musicBtnHolder
    browseBtn.Size = UDim2.new(0, 60, 1, 0)
    browseBtn.Position = UDim2.new(0, 65, 0, 0)
    browseBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
    browseBtn.Text = "Browse"
    browseBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    browseBtn.TextSize = 9
    browseBtn.Font = Enum.Font.GothamBold
    browseBtn.AutoButtonColor = false
    browseBtn.MouseButton1Click:Connect(function()
        local files = Functions:BrowseFiles()
        if files and #files > 0 then
            createPopup("Select Music File", files, function(selected)
                Functions:PlayMusic(selected)
            end)
        end
    end)
    ...
    stopBtn.Position = UDim2.new(0, 130, 0, 0)
