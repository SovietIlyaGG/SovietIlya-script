...
function Functions:SpawnBrainrot(rarity, amount)
    ...
    print("Spawning " .. rarity .. " x" .. amount)
    for i = 1, amount do
        pcall(function()
            local args = {[1] = rarity}
            for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
                if remote:IsA("RemoteEvent") and (remote.Name:lower():find("spawn") or remote.Name:lower():find("brainrot") or remote.Name:lower():find("give")) then
...
function Functions:StartEvent(eventName)
    ...
    pcall(function()
        for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
            if remote:IsA("RemoteEvent") and (remote.Name:lower():find("event") or remote.Name:lower():find("start")) then
...
function Functions:LuckBoost(on)
    if on then
        luckLoop = RunService.Heartbeat:Connect(function()
            pcall(function()
                for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
                    if remote:IsA("RemoteEvent") and (remote.Name:lower():find("luck") or remote.Name:lower():find("boost")) then
...
function Functions:GiveSeeds(name, amount)
    amount = amount or 999
    print("Giving " .. name .. " x" .. amount)
    for i = 1, amount do
        pcall(function()
            local args = {[1] = name}
            for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
                if remote:IsA("RemoteEvent") and (remote.Name:lower():find("seed") or remote.Name:lower():find("harvest") or remote.Name:lower():find("plant") or remote.Name:lower():find("give")) then
                    remote:FireServer(unpack(args))
                end
            end
        end)
...
function Functions:GivePet(name, amount)
    ...
    for i = 1, amount do
        pcall(function()
            local args = {[1] = name}
            for _, remote in ipairs(ReplicatedStorage:GetDescendants()) do
                if remote:IsA("RemoteEvent") and (remote.Name:lower():find("pet") or remote.Name:lower():find("hatch") or remote.Name:lower():find("give") or remote.Name:lower():find("egg")) then
                    remote:FireServer(unpack(args))
                end
            end
        end)
